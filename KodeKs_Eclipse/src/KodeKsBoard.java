...
public class KodeKsBoard extends KodeKsData implements ActionListener, MouseListener{
	...
	protected static JLabel message; // a message label which shows who's turn it is
	
	protected JPanel Player1_Stones; //On these labels are the taken Stones
	protected JPanel Player2_Stones;
	protected static JLabel Player1Name; //On these Labels are the names of the two players
	protected static JLabel Player2Name;
	
	static int NumberOfRedStones=0; //these inegers represents the left stones of each player
	static int NumberOfBlueStones=0;
	protected static JPanel board; //This panel contains the board, the statuspanel and the "stonepanels"

	boolean gameInProgress; // Is a game currently in progress?

/* The next five variables are valid only when the game is in progress. */

public static int currentPlayer;      // Whose turn is it now?  The possible values are 1(red player) and 2(blue player)
...
ThreatenStone[] getThreatenStones;	 // An array containing the threaten stones by the current player.

KodeKsMove[] legalMoves; 	 // An array containing the legal moves for the current player.
	

/**
* Constructor. Create the labels, the board and starts a new game.  Listens for mouse.
* clicks
* @param parent - Gui 
...
getThreatenStones = getThreatenStone(KodeKsData.RED); //Get the BLUE stones which are threaten by RED (is empty at the beginning)
...
Get that player's ThreatenStones, legal moves and the FoolStoneCombinations of the opponent. If the player has less the 5 stones,
then the game ends. */
...
/**
* (Draw a KodeKsboard pattern in gray and lightGray. Draw the
* pieces.) Load the images of the board and the pieces. Draw the names of rows and columns
* If a game is in progress, hilite the legal moves and the pieces to take
* @param g - Graphics
*/
...
for (int row = 0; row < 10; row++) {
	for (int col = 0; col < 10; col++) {
		...
		Font value=new Font("Gothic", Font.BOLD, 24);
		g.setFont(value);
...
/* If a game is in progress, hilite the legal moves and the possible Pieces to take. Note that legalMoves
